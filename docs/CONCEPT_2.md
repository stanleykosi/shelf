...
The accepted design is the only frontend at `/`, `/discover`, and the related routes. The historical comparison UI and query-based concept selection have been removed. No new authentication mode or financial capability is introduced by this visual direction.
...
| Type | Raleway, regular-weight headlines; 40–90px opening title, 36–58px section headings, 14–18px body |
...
- Home, Discover, Scan, Scan Results, research, Saved, Assistant and operations use the accepted visual system on their canonical routes.
- Discover retains current issuer directory data, filters, search, and pagination behind its editorial layout.
- The old landing composition and comparison selector are no longer rendered or linked.
...
`artifacts/concept-2/` contains historical 1440px, 1280px, 430px and 390px Home/Discover captures, search and filtered Discover states, and Home motion recordings. The capture script now targets canonical routes only. Generate fresh captures against a local server with:

```sh
PLAYWRIGHT_BASE_URL=http://127.0.0.1:3100 node scripts/capture-design-concepts.mjs
```

Browser regressions cover canonical navigation, retained search context, product-example links, reduced motion, pause/resume, keyboard filter behavior, reloadable filter state, 390/430/1280/1440 layouts, and mobile bottom-nav clearance.
