...
4. Run `npm run prepare:railway-function`, link `.railway/worker.generated.ts` to `shelf-worker` if needed, and run `railway functions push --path .railway/worker.generated.ts`. This generated, ignored one-file artifact pins the Postgres version from `package.json`. Confirm one successful issuer refresh, including both `issuer_feed_snapshots` rows and `discover_warmed:2` in the worker result. The public directory falls back to provider reads until snapshots exist, and labels snapshots stale after ten minutes without a refresh. Warm the production alias immediately after each Vercel deployment by reading `/discover` and `/api/v1/issuer/directory`; the scheduled worker maintains warm paths afterward.
