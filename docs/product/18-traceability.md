...
| U38 asset chart and purchase polish | PR04, PR08, PR16, PR24 | chart range selects its initial viewport; panning requests older observed issuer-pool candles and preserves the viewport; copy mint shows a temporary check icon; the amount field groups digits and blocks review below 5 USDC | provider pagination, repeated browser pan, copy icon state, minimum purchase and server `ORDER_LIMIT` checks |
