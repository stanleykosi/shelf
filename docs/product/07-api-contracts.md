...
| GET /issuer/asset/xstocks/{symbol}/market?range=1D\|1W\|1M\|3M[&before=unixSeconds] | guest/member | exact current issuer mint selects a qualifying Solana pool via DEX Screener; returns indicative pool price, 24-hour move and GeckoTerminal USD candle pages for that mint and pool. `before` requests older observed candles while the chart pans; the client merges only matching pool pages and retains its viewport. `historyState` distinguishes available, empty and provider error; successful responses cache 180 seconds, provider errors are not cached; history is never synthetic or executable |
...
- POST /ai/answer: {question,issuer:{provider:xstocks|prestocks,symbol},history?:[{role:user|assistant,content}]} or {question,scope:general,history?}. Member or limited guest. An exact issuer reference or explicit general scope is required; missing/invalid scope returns INVALID_INPUT before provider work. Asset answers refetch the exact current issuer record; general answers use only reviewed learning articles and product relationships, not live issuer prices. History is at most six prior turns of 1,000 characters each. The server does not persist chat text; browser history is separated by account/guest tab and has explicit deletion. Returns {answer,sourceIds,uncertainty,issuer} with issuer null for general chat. Every request enforces OpenRouter privacy routing and shared spend limits. The current transport returns one bounded JSON answer, with browser abort as best-effort cancellation.
