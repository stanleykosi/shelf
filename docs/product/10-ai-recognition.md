...
Asset chat is entered on the token detail page or at `/assets/[provider]/[symbol]/chat`. Provider and symbol select the exact current xStocks or PreStocks record on the server. The prompt receives its complete bounded public issuer JSON, the validated normalized listing, source URL, observation time and any reviewed lifecycle notice. The browser sends only provider, symbol, question and bounded prior turns; it cannot author issuer facts. Each turn refetches the exact record. General chat at `/assistant` sends explicit `scope:general` and receives reviewed Shelf learning and product relationship facts, without current issuer prices or mints. Earlier turns help resolve follow-up wording but never become factual sources. Opening chat does not invoke OpenRouter; only a submitted question does. A request missing both exact issuer context and explicit general scope is rejected before provider work. A signed, HttpOnly guest cookie separates browser quotas without storing chat content, while the shared budget and network cap remain. Chat transcripts stay out of server usage logs; signed-in history is local to that account in the browser for up to 90 days, and guest history remains in the current tab. Each thread can be deleted.
