...
describe("screen action inventory", () => {
  ...
  it("connects current issuer details to the verified purchase route", () => {
    const issuerScreen = readFileSync("src/components/screens/issuer-assets.tsx", "utf8");
    const apiRoute = readFileSync("src/app/api/v1/[...path]/route.ts", "utf8");

    expect(issuerScreen).toContain("/assets/${provider}/${encodeURIComponent(asset.symbol)}/buy");
