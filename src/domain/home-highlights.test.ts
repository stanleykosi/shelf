...
function pool(mint: string, liquidityUsd: number, change1hPct: number, priceUsd = "140.25") {
  return {
    chainId: "solana", dexId: "raydium", pairAddress: `pair-${liquidityUsd}`,
    baseToken: { address: mint }, priceUsd,
    priceChange: { h1: change1hPct }, liquidity: { usd: liquidityUsd },
  };
}
...
describe("home market highlights", () => {
  it("uses only exact verified Solana mints and the deepest qualifying pool", () => {
    ...
    expect(markets.find((market) => market.mint === appleMint)).toMatchObject({ change1hPct: -4, liquidityUsd: 90_000 });
  });

  it("sorts current company listings by one-hour change without inventing missing prices", () => {
    const markets = selectPoolMarkets([
    ...
    expect(highlights[0]).toMatchObject({ priceUsd: "140.25", change1hPct: 4 });
  });
});
